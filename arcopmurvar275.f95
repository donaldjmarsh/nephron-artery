!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   !   program to vary exponent value in Murray's Law application
...
!  Initialize time and counters.
      ...
      iterateradii: do while (lcount .lt. 3)
       ...
       if (myid .eq. 0) then
          ...
          open(unit=20,FILE='mtn0'//trim(nr)//irun)
       endif


        if (myid .eq. 1) then
        call outname(irun,nr)
         open(lout(1),FILE='mtn'//trim(nyid(myid))//trim(nr)//irun)
         endif

         if (myid .eq. 2) then
            call outname(irun,nr)
            open(lout(2),FILE='mtn'//trim(nyid(myid))//trim(nr)//irun)
         endif

         if (myid .eq. 3) then
            call outname(irun,nr)
            open(lout(3),FILE='mtn'//trim(nyid(myid))//trim(nr)//irun)
         endif

         if (myid .eq. 4) then
            call outname(irun,nr)
            open(lout(4),FILE='mtn'//trim(nyid(myid))//trim(nr)//irun)
         endif

         if (myid .eq. 5) then
            call outname(irun,nr)
            open(lout(5),FILE='mtn'//trim(nyid(myid))//trim(nr)//irun)
         endif

          if (myid .eq. 6) then
            call outname(irun,nr)
            open(lout(6),FILE='mtn'//trim(nyid(myid))//trim(nr)//irun)
            endif
         
          if (myid .eq. 7) then
            call outname(irun,nr)
            open(lout(7),FILE='mtn'//trim(nyid(myid))//trim(nr)//irun)
           endif

          if (myid .eq. 8) then
            call outname(irun,nr)
            open(lout(8),FILE='mtn'//trim(nyid(myid))//trim(nr)//irun)
           endif


          if (myid .eq. 9) then
            call outname(irun,nr)
            open(lout(9),FILE='mtn'//trim(nyid(myid))//trim(nr)//irun)
           endif

          if (myid .eq. 10) then
            call outname(irun,nr)
            open(lout(10),FILE='mtn'//trim(nyid(myid))//trim(nr)//irun)
           endif
