...
!_______________________________________________________________________
        ...
        SUBROUTINE DIFFUN(N,T,y,yp)  
	use affall
	use dvode_f90_m
        implicit NONE

        integer N,klm,i,j,k,nodeN,jkl
        real*8 v3,poteq,xlambda,xm,rad
	
	real*8 y(N),yp(N)
        real*8 psi,p1,p2,p3,xpr,xs,t,xpsi,xtgf
	real*8 sigx,sigy,sigu,u,fdelp,fx,fu
        real*8 gcb 

         segaff: do i = 1,numberOfNephrons
          segres(i,1)=kr(1)/(affRad(i,1)**4.D0)
          segres(i,2)=segres(i,1)+kr(2)/(affRad(i,2)**4.D0)

            do klm =1,2
             apress(i,klm)=pressNod(nodeNumber(i))/dyn- &
              (pressNod(nodeNumber(i))/dyn-pressGlom(i))* &
             (segres(i,klm-1)+segres(i,klm))/(2.D0*segres(i,2))
            enddo
	  ...
	  rasAff(i)=segres(i,2)
        ...
        do i = 1,9
            do j=1,11
               Vec3(j)=Matrix3(i,j)
            enddo
            potNode(i)=-dot_product(Vec3,Vec1)
         enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!  First (farthest from glomerulus) arteriolar segment
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

          !  potassium ode

        v3=v6 -(v5/2.D0)*tanh((y(j+2)-ca3)/ca4)
        poteq=0.5D0*(1. + tanh((y(j+3)-v3)/v4))
	xlambda = phin(i)*cosh((y(j+3)-v3)/(2.D0*v4))
        yp(j+1)=xlambda*(poteq-y(j+1))

        xm=0.5D0*(1.D0 + tanh((y(j+3)-v1)/v2))
	gcb=(1.0D0 + v7*xtgf(yt(i),1))*gca
        rho=((kd+y(j+2))**2.D0)/((kd+y(j+2))**2.D0+kd*bt)
        yp(j+2)=(-alpha*gcb*xm*(y(j+3)-vca) - kca*y(j+2))*rho

        !  PD ode

	p1=gel*(y(j+3)-vel)
        p2=gk*y(j+1)*(y(j+3)-vk)
        p3=gcb*xm*(y(j+3)-vca)
        yp(j+3)=(-p1 - p2 - p3 - gpd*(y(j+3) - y(j+9))) - gnef*(y(j+3)-potNode(PDnodeNumber(i)))

        ! muscle phoshorylation

	psi=(y(j+2)**qm)/((caim**qm)+(y(j+2)**qm))
        yp(j+4)=xkpsi*((psi/(psim+psi))-y(j+4))

        ! vessel mechanics -  circumference

        xpr=y(j+5)/x0

        sigx=x3*(1.D0+tanh((xpr-x1)/x2))+x4*(xpr-x5) &
       -x8*(x6/(xpr-x7))**2.D0 - x9
        u = (y(j+5) - y(j+6))/x0
        sigu=(u2*exp(u1*u))-u3
        fdelp=.5D0*apress(i,1)*((y(j+5)/xpi)-Area/y(j+5))
        fx=we*S*sigx*sig0
        fu=wm*S*sigu*sig0
        yp(j+5)=(1.D0/tau)*(fdelp-fx-fu)

        ! contractile element

        xs=2.0D0*(((y1/((y(j+6)/x0)+y2))**y4)**2.0D0)
        sigy=(y(j+4)/xomref)*(exp((-((y(j+6)/x0)-yx)**2.D0)/xs)-y3)/ &
        (1.0D0-y3)
        if (sigu/sigy .le. 1.0D0) then
        yp(j+6)=x0*(-xnuref*(psi/psiref)*ax*((1.0D0 -sigu/sigy) &
        /(ax+sigu/sigy)))
        else
          yp(j+6)=x0*cx*(exp(bx*((sigu/sigy)-dx)) &
          -exp(bx*(1.0D0-dx)))
        endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!   Second (closest to glomerulus arteriolar segment
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        v3=v6 -(v5/2.D0)*tanh((y(j+8)-ca3)/ca4)
        poteq=0.5D0*(1. + tanh((y(j+9)-v3)/v4))
	xlambda = phin(i)*cosh((y(j+9)-v3)/(2.D0*v4))
        yp(j+7)=xlambda*(poteq-y(j+7))

        !  calcium ode


        xm=0.5D0*(1.D0 + tanh((y(j+9)-v1)/v2))
	gcb=(1.0D0 + v7*xtgf(yt(i),2))*gca
        rho=((kd+y(j+8))**2.D0)/((kd+y(j+8))**2.D0+kd*bt)
        yp(j+8)=(-alpha*gcb*xm*(y(j+9)-vca) - kca*y(j+8))*rho
        gcc(i)=1.0D0 + v7*xtgf(yt(i),2)
        !  PD ode


	p1=gel*(y(j+9)-vel)
        p2=gk*y(j+7)*(y(j+9)-vk)
        p3=gcb*xm*(y(j+9)-vca)
        yp(j+9)=(-p1 - p2 - p3 - gpd*(y(j+9) - y(j+3)))


        ! muscle phoshorylation

	psi=(y(j+8)**qm)/((caim**qm)+(y(j+8)**qm))
        yp(j+10)=xkpsi*((psi/(psim+psi))-y(j+10))
        ...
        u = (y(j+11) - y(j+12))/x0
        sigu=(u2*exp(u1*u))-u3
        fdelp=.5D0*apress(i,2)*((y(j+11)/xpi)-Area/y(j+11))
        fx=we*S*sigx*sig0
        fu=wm*S*sigu*sig0
        yp(j+11)=(1.D0/tau)*(fdelp-fx-fu)

        ! contractile element


        xs=2.0D0*(((y1/((y(j+12)/x0)+y2))**y4)**2.0D0)
        sigy=(y(j+10)/xomref)*(exp((-((y(j+12)/x0)-yx)**2.D0)/xs)-y3)/ &
       (1.0D0-y3)
        if (sigu/sigy .le. 1.0D0) then
        yp(j+12)=x0*(-xnuref*(psi/psiref)*ax*((1.0D0 -sigu/sigy) &
        /(ax+sigu/sigy)))
        else
          yp(j+12)=x0*cx*(exp(bx*((sigu/sigy)-dx)) &
         -exp(bx*(1.0D0-dx)))
        endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 function xtgf(x,n)

!  Function to determine TGF activation in each arteriolar segment
