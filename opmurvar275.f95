...
!  Initialize time and counters.
      ...
      iterateradii: do while (lcount .lt. 3)
       ...
       if (myid .eq. 0) then
          ...
          open(unit=20,FILE='ntm0'//trim(nr)//irun)
       endif


        if (myid .eq. 1) then
        call outname(irun,nr)
         open(lout(1),FILE='ntm'//trim(nyid(myid))//trim(nr)//irun)
         endif

         if (myid .eq. 2) then
            call outname(irun,nr)
            open(lout(2),FILE='ntm'//trim(nyid(myid))//trim(nr)//irun)
         endif

         if (myid .eq. 3) then
            call outname(irun,nr)
            open(lout(3),FILE='ntm'//trim(nyid(myid))//trim(nr)//irun)
         endif

         if (myid .eq. 4) then
            call outname(irun,nr)
            open(lout(4),FILE='ntm'//trim(nyid(myid))//trim(nr)//irun)
         endif

         if (myid .eq. 5) then
            call outname(irun,nr)
            open(lout(5),FILE='ntm'//trim(nyid(myid))//trim(nr)//irun)
         endif

          if (myid .eq. 6) then
            call outname(irun,nr)
            open(lout(6),FILE='ntm'//trim(nyid(myid))//trim(nr)//irun)
            endif
         
          if (myid .eq. 7) then
            call outname(irun,nr)
            open(lout(7),FILE='ntm'//trim(nyid(myid))//trim(nr)//irun)
           endif

          if (myid .eq. 8) then
            call outname(irun,nr)
            open(lout(8),FILE='ntm'//trim(nyid(myid))//trim(nr)//irun)
           endif


          if (myid .eq. 9) then
            call outname(irun,nr)
            open(lout(9),FILE='ntm'//trim(nyid(myid))//trim(nr)//irun)
           endif

          if (myid .eq. 10) then
            call outname(irun,nr)
            open(lout(10),FILE='ntm'//trim(nyid(myid))//trim(nr)//irun)
           endif
