...
!  afferent arteriolar segments solved with dvode
       ...
       integer :: i,j,k,klm,lmn,mno,jcount,kcount,lcount,mcount,ipcount
...
!  dimensions of rtol and atol must be changed with numberOfNephrons
       ...
       real(kind=8) :: flowsum, flow1,flow23,flow4,flow5,flow67
       real(kind=8) :: flow8,flow910,xdist
       real(kind=8) :: ydist,pydist 
...
!      Assign node numbers for nephron PD calculations
       ...
       call MPI_BCAST(res, 1, MPI_DOUBLE_PRECISION, 0, &
            MPI_COMM_WORLD, ierr )
...
!  Initialize time and counters.
       ...
       nodemeandiff(9)=.245D0
              ...
              xdist=1.0D0;lcount=1;ydist=1.0D0;ncount=0;pydist =.02D0;bpcount=0
!              xdist=1.0D0;ydist=1.0D0;ncount=0;pydist =.02D0;bpcount=0 !lcount=1;
              lflag=0;ipcount = 0
      iterateradii: do while (lcount .le. 3)
       ...
       OPEN0:if (myid .eq. 0) then
          call outname(irun,nr)
          open(unit=20,FILE='ntv0'//trim(nr)//irun)
       endif OPEN0


        OPEN1:if (myid .eq. 1) then
          call outname(irun,nr)
          open(lout(1),FILE='ntv'//trim(nyid(myid))//trim(nr)//irun)
        endif OPEN1

        OPEN2: if (myid .eq. 2) then
            call outname(irun,nr)
            open(lout(2),FILE='ntv'//trim(nyid(myid))//trim(nr)//irun)
         endif OPEN2

         OPEN3:if (myid .eq. 3) then
            call outname(irun,nr)
            open(lout(3),FILE='ntv'//trim(nyid(myid))//trim(nr)//irun)
         endif OPEN3

         OPEN4:if (myid .eq. 4) then
            call outname(irun,nr)
            open(lout(4),FILE='ntv'//trim(nyid(myid))//trim(nr)//irun)
         endif OPEN4

         OPEN5:if (myid .eq. 5) then
            call outname(irun,nr)
            open(lout(5),FILE='ntv'//trim(nyid(myid))//trim(nr)//irun)
         endif OPEN5

         OPEN6:if (myid .eq. 6) then
            call outname(irun,nr)
            open(lout(6),FILE='ntv'//trim(nyid(myid))//trim(nr)//irun)
            endif OPEN6
         
         OPEN7:if (myid .eq. 7) then
            call outname(irun,nr)
            open(lout(7),FILE='ntv'//trim(nyid(myid))//trim(nr)//irun)
           endif OPEN7

         OPEN8:if (myid .eq. 8) then
            call outname(irun,nr)
            open(lout(8),FILE='ntv'//trim(nyid(myid))//trim(nr)//irun)
           endif OPEN8


          OPEN9:if (myid .eq. 9) then
            call outname(irun,nr)
            open(lout(9),FILE='ntv'//trim(nyid(myid))//trim(nr)//irun)
           endif OPEN9

          OPEN10:if (myid .eq. 10) then
            call outname(irun,nr)
            open(lout(10),FILE='ntv'//trim(nyid(myid))//trim(nr)//irun)
           endif OPEN10
     ...
     ! calculate nephron blood flows for network vascular node values
	...
	jcount=jcount+1
        ...
        if (myid == 0) then
           ...
           SUMFLOWS:do mno=1,numberOfNephrons
           ...
           if (lcount == 2)then
              rootpress = dyn*(85.0D0 + (dble(inr)-85.0D0)*tanh(sysTime/40.D0))
         endif


           if (lcount == 3) then
              ipcount =ipcount+1
           if (ipcount == 1000) then
             bpcount = bpcount + 1
             rootpress = dyn*rootbp(bpcount)
             ipcount = 0
          endif
          endif
        ...
        ! update afferent arteriolar radii
	j=12
...
!     avrad(i) is the radius of the ith afferent arteriole
      ...
      if (myid == 0) then
      call AVRADCHECK(kcount)

      if (lcount == 1) then

         artgRad1=bifuradadd(artRad(2),artRad(4))
         artgRad2=bifuradadd(avrad(1),artRad(3))
         artgRad3=bifuradadd(avrad(2),avrad(3))
         artgRad4=bifuradadd(avrad(4),artRad(5))
         artgRad5=bifuradadd(avrad(5),artRad(6))
         artgRad6=bifuradadd(artRad(7),artRad(8))
         artgRad7=bifuradadd(avrad(7),avrad(6))
         artgRad8=bifuradadd(avrad(8),artRad(9))
         artgRad9=bifuradadd(avrad(9),avrad(10))
...
!      endif NUPHASE
      ...
      NUPHASE: if (ydist .le. pydist .and. lcount == 1) then
         lcount = 2
         ncount = 1
         lflag = 1
      else if (lcount == 2 .and. sysTime .ge. timestop(2)) then
         lcount = 3
      else if (lcount == 3 .and. sysTime .ge. timestop(3)) then
         lcount = 4
      endif NUPHASE
      if (ncount == 1) write (*,*) sysTime,jcount,lflag,lcount
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         subroutine ydUpdate(ydist)

           use affall

           implicit NONE

           real(kind=8) ydist !,nodemeandiff(9),nodeadd(9)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!                                      
        function bifuradadd(x1,x2)
          ...
          bifuradadd=(x1**x3+x2**x3)**(1.D0/x3)
