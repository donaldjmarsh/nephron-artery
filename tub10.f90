...
!        local scalars
         ...
         real*8, private :: xrad
         real*8, private :: pt10
         real*8, private :: euknorm10
         real*8, private :: amm10
         ...
         real*8, private, parameter :: bmm10=-0.1631D0
         real*8, private, parameter :: cmm10=-.00294D0
         ...
         integer, parameter, private :: nefid = 10
...
!         private subroutines and functions
          ...
          bitri10    ,BOUND10    ,dfdpg10    ,feedb10   ,     &
          glom10     ,LUBKSB10   ,ludcmp10   ,f10       ,     &
          MNEWT10    ,MMNEWT10   ,nephron10  ,prflcl10  ,     &
          RADIUS10   ,REABGRAD10 ,tubgeom10  ,PDE10     ,     &
          chlorid10  ,stm110     ,stm210     ,stm310    ,     &
          stm410     ,stm510

!! Beginning of tub10 interface.                                                                 
!! ..                                                                                                
!! .. Generic Interface Blocks ..                                                                    
      INTERFACE tub10_F90

!        initall10 downloads all tubular parameters and initial conditions

         MODULE PROCEDURE initall10 

!        calc10 runs the calculations for the tubule

         MODULE PROCEDURE calc10
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	xtub10(1)=35.1D0;xtub10(2)=106.9D0;xtub10(3)=55.D0
	xtub10(4)=0.209D0;xtub10(5)=.59D0;xtub10(6)=81.5D0
	xtub10(7:15)=0.0D0
	...
	return
	end subroutine initall10
!!!!!!!!!!!!!!!!!!!!!!!!!
        subroutine calc10(tubTime,jcount)
...
!        ireq(X) X= 2*10
      ...
      itstep=0;euknorm10=1.D0;ras=1.0
      ...
      xt2=feedb10(nacl(imd,1),nacl(imd,2))
	...
	sbf=xtub10(2);signal=xt2

	bufSend10(1)=sbf;bufSend10(2)=signal;bufSend10(3)=gpress


!  ireq(X) X related to send sequence

        call MPI_ISEND(bufSend10,3,MPI_DOUBLE_PRECISION, &
          0,10,MPI_COMM_WORLD,ireq(19),ierr)

 	if (jflag .eq. 0) then
     if (jcount.EQ. iprint) then
          if (tubTime.GE.startprt) then   
	     ytime=tubTime-startprt
            gfr=xtub10(1);spf=xtub10(2)
            tpress=p(ilen(1),1)*conv1;xmacd=nacl(imd,1)*1.0d03

            write(lout(nefid),'(f8.3,5(2x,g12.4))') ytime ,tpress,xmacd,2.0D0*spf, &
                 pressNod(nodeNumber(10))/dyn, (nacl(imd,1)+nacl(imd,2))/2.0D0
...
!  ireq(X) X related to send sequence
	...
	call MPI_WAIT(ireq(19),status,ierr)


      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine nephron10
      use affall
      use tuball
...
! Start iteration loop. 
      ...
      itstep=0;euknorm10=1.D0;sum(3)=1.D0

      do while (((euknorm10.GT.1D-3).OR.(sum(3).GT.1.D-5)) &
...
!
      ...
      call MMNEWT10(ntrial,xtub10,n,tolx,tolf)

      q(0,2)=xtub10(1)*conv2
...
!  Send message and stop if iteration counter exceeds maxstep.
	...
	enddo
         stop 'itstep exceeds maxstep in nephron10'
      endif
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  newt using the Newton-Raphson method. 
          ...
          call mnewt10(ntrial,x,n,tolx,tolf)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine BOUND10(x,alpha,beta)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MNEWT10(ntrial,x,n,tolx,tolf)
...
!
      ...
      first: do k=1,ntrial
	   ...
	   enddo second
          ...
          call ludcmp10(alpha,n,np,indx) 
          call lubksb10(alpha,n,np,indx,beta)
          errx=0.D0
          third: do i=1,n
	...
	enddo first
        stop 'MNEWT10 exceeding max number of iterations'
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MMNEWT10(ntrial,x,n,tolx,tolf)
	  ...
	  enddo second
          ...
          xp=amm10+bmm10*x(6)+cmm10*x(6)*x(6) 
      ...
      write (6,*) xp
      stop 'MMNEWT10 exceeding max number of iterations'

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine ludcmp10(a,n,np,indx) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine LUBKSB10(a,n,np,indx,b)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION feedb10(nacl1,nacl2)
...
!     TGF response of macula densa and afferent arteriole
      ...
      feedb10=tgfmax-dtgf/(1.D0+dexp(xgain*(nacl-tpnacl)))

      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION f10(ce,gpf)
...
!      cf=-0.00294D0
      ...
      1.D0/(af*ce)+(bf*bf/(2.D0*af*af)-cf/af)*stm110(ce,af,bf,cf)

      integ2=(bf/(2.D0*af*af))*log((af+bf*ca+cf*ca*ca)/(ca*ca))- &
      1.D0/(af*ca)+(bf*bf/(2.D0*af*af)-cf/af)*stm110(ca,af,bf,cf)

      f10=integ1-integ2-kf/(gpf*ca)


      return
      end 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION dfdpg10(CA,CE)  
...
!  Calculation of integral of 1/C^2*(PG-PT-PI(C))^2 in [CA,CE]
      ...
      a=gpress-pt10
      integ1=stm510(ce,a,bf,cf)
      integ2=stm510(ca,a,bf,cf)
      integ=integ1-integ2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     subroutine glom10(xtub,alfa,beta)
      use affall
...
!    to 0 in this version).
      ...
      beta(2)=gpf-(1.-ha)*(pressNod(nodeNumber(10))/dyn-gpress)/(ras*exp(ks*ha))
      beta(3)=gpress-re*(gpf/(1-ha)-sngfr)-gpf*rv/(1-ha)
      beta(4)=re-(res+acomp*(gpress-pg0))*exp(ks*he)
      ...
      alfa(4,4)=-1.D0
      alfa(4,5)=(res+acomp*(gpress-pg0))*ks*exp(ks*he)
      alfa(5,5)=-1.D0
      alfa(5,6)=(ca*((1.D0-ha)/ha)/(ce+ca*((1.D0-ha)/ha))**2)
      alfa(6,2)=-kf/(ca*gpf*gpf)
      alfa(6,3)=-dfdpg10(ca,ce)
      alfa(6,6)=-1.D0/(ce*ce*(gpress-pt10-aa*ce-bb*ce*ce))

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      Real*8 FUNCTION stm110(x,a,b,c)
...
!   hvor X = ( a + bx + cx^2 ). q = 4ac-b^2. 
      ...
      stm110=(-1.D0/sqrt(-q))*log((2.D0*c*x+b+sqrt(-q))/ &
       (sqrt(-q)-2.D0*c*x-b))
 
      end
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm210(x,a,b,c)

      real*8 x,a,b,c 

      stm210=(1.D0/(2.D0*a))*log(x*x/(a+b*x+c*x*x))- &
      (b/(2.D0*a))*stm110(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      real*8 FUNCTION stm310(x,a,b,c)
      real*8 x,a,b,c,q
  
      q=4.D0*a*c-b*b
      stm310=(2.D0*c*x+b)/(q*(a+b*x+c*x*x)) + &
      (2.D0*c/q)*stm110(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm410(x,a,b,c)

      real*8 x,a,b,c
  
      stm410=1.D0/(2.D0*a*(a+b*x+c*x*x))-(b/(2.D0*a))* &
      stm310(x,a,b,c)+stm210(x,a,b,c)/a
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm510(x,a,b,c)

      double precision :: x,a,b,c 
  
      stm510=-1.D0/(a*x*(a+b*x+c*x*x))-(2.D0*b/a)*  &
      stm410(x,a,b,c)-(3.D0*c/a)*stm310(x,a,b,c)
