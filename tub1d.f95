...
!        local scalars
         ...
         real*8, private, parameter :: bmm1=-0.1631D0
         real*8, private, parameter :: cmm1=-.00294D0
         ...
         integer, parameter, private :: nefid = 1
...
!         private subroutines and functions
          ...
          bitri1    ,BOUND1    ,dfdpg1    ,feedb1   ,     &
          glom1     ,LUBKSB1   ,ludcmp1   ,f1       ,     &
          MNEWT1    ,MMNEWT1   ,nephron1  ,prflcl1  ,     &
          RADIUS1   ,REABGRAD1 ,tubgeom1  ,PDE1     ,     &
          chlorid1  ,stm11     ,stm21     ,stm31    ,     &
          stm41     ,stm51

!! Beginning of tub1 interface.                                                                 
!! ..                                                                                                

          !! .. Generic Interface Blocks ..                                                                    
      INTERFACE tub1_F90

!        initall1 downloads all tubular parameters and initial conditions

         MODULE PROCEDURE initall1 

!        calc1 runs the calculations for the tubule

         MODULE PROCEDURE calc1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	xtub1(1)=35.1D0;xtub1(2)=106.9D0;xtub1(3)=55.D0
	xtub1(4)=0.209D0;xtub1(5)=.59D0;xtub1(6)=81.5D0
	xtub1(7:15)=0.0D0
	...
	return
	end subroutine initall1
!!!!!!!!!!!!!!!!!!!!!!!!!
        subroutine calc1(tubTime,jcount)
...
!        ireq(X) X= 2*1
      ...
      itstep=0;euknorm1=1.D0;ras=1.0 
      ...
      xt2=feedb1(nacl(imd,1),nacl(imd,2))
	...
	sbf=xtub1(2);signal=xt2

	bufSend1(1)=sbf;bufSend1(2)=signal;bufSend1(3)=gpress


!  ireq(X) X related to send sequence

        call MPI_ISEND(bufSend1,3,MPI_DOUBLE_PRECISION, &
          0,1,MPI_COMM_WORLD,ireq(1),ierr)
...
!  ireq(X) X related to send sequence
	...
	call MPI_WAIT(ireq(1),status,ierr)
...
!      do i=1,irlen+1; write (*,*) i, q(i-1,1); enddo
!      write (*,*) u(irlen)
!                  stop
...
! Start iteration loop. 
      ...
      itstep=0;euknorm1=1.D0;sum(3)=1.D0

      do while (((euknorm1.GT.1D-3).OR.(sum(3).GT.1.D-5)) &
...
!
      ...
      call MMNEWT1(ntrial,xtub1,n,tolx,tolf)
...
!  Send message and stop if iteration counter exceeds maxstep.
	...
	enddo
         stop 'itstep exceeds maxstep in nephron1'
      endif
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  newt using the Newton-Raphson method. 
          ...
          call mnewt1(ntrial,x,n,tolx,tolf)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine BOUND1(x,alpha,beta)
...
!  Proximal tubule
     ...
     if (pressNod(nodeNumber(1))/dyn .gt. 85.0D0) then
         xdelp = (pressNod(nodeNumber(1))/dyn - 85.D0)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MNEWT1(ntrial,x,n,tolx,tolf)
...
!
      ...
      first: do k=1,ntrial
	   ...
	   enddo second
          ...
          call ludcmp1(alpha,n,np,indx) 
          call lubksb1(alpha,n,np,indx,beta)
          errx=0.D0
          third: do i=1,n
	...
	enddo first
        stop 'MNEWT1 exceeding max number of iterations'
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MMNEWT1(ntrial,x,n,tolx,tolf)
      ...
      write (6,*) xp
      stop 'MMNEWT1 exceeding max number of iterations'

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine ludcmp1(a,n,np,indx) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine LUBKSB1(a,n,np,indx,b)
...
!      write (*,*) q(ilen(1),1) !q(0,2) !u(irlen) !
!      stop
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION feedb1(nacl1,nacl2)
...
!     TGF response of macula densa and afferent arteriole
      ...
      feedb1=tgfmax-dtgf/(1.D0+dexp(xgain*(nacl-tpnacl)))

      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION f1(ce,gpf)
...
!      cf=-0.00294D0
      ...
      1.D0/(af*ce)+(bf*bf/(2.D0*af*af)-cf/af)*stm11(ce,af,bf,cf)

      integ2=(bf/(2.D0*af*af))*log((af+bf*ca+cf*ca*ca)/(ca*ca))- &
      1.D0/(af*ca)+(bf*bf/(2.D0*af*af)-cf/af)*stm11(ca,af,bf,cf)

      f1=integ1-integ2-kf/(gpf*ca)


      return
      end 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION dfdpg1(CA,CE)  
...
!  Calculation of integral of 1/C^2*(PG-PT-PI(C))^2 in [CA,CE]
      ...
      a=gpress-pt1
      integ1=stm51(ce,a,bf,cf)
      integ2=stm51(ca,a,bf,cf)
      integ=integ1-integ2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     subroutine glom1(xtub,alfa,beta)
      use affall
...
!    to 0 in this version).
      ...
      zres = pressNod(nodeNumber(1))/dyn
      if (zres .gt. 85.0D0) then
         yres = (zres - 85.D0)/25.D0
      ...
      beta(2)=gpf-(1.-ha)*(pressNod(nodeNumber(1))/dyn-gpress)/(ras*exp(ks*ha))
      ...
      alfa(6,3)=-dfdpg1(ca,ce)
      alfa(6,6)=-1.D0/(ce*ce*(gpress-pt1-aa*ce-bb*ce*ce))

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      Real*8 FUNCTION stm11(x,a,b,c)
...
!   hvor X = ( a + bx + cx^2 ). q = 4ac-b^2. 
      ...
      stm11=(-1.D0/sqrt(-q))*log((2.D0*c*x+b+sqrt(-q))/ &
       (sqrt(-q)-2.D0*c*x-b))
 
      end
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm21(x,a,b,c)

      real*8 x,a,b,c 

      stm21=(1.D0/(2.D0*a))*log(x*x/(a+b*x+c*x*x))- &
      (b/(2.D0*a))*stm11(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      real*8 FUNCTION stm31(x,a,b,c)
      real*8 x,a,b,c,q
  
      q=4.D0*a*c-b*b
      stm31=(2.D0*c*x+b)/(q*(a+b*x+c*x*x)) + &
      (2.D0*c/q)*stm11(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm41(x,a,b,c)

      real*8 x,a,b,c
  
      stm41=1.D0/(2.D0*a*(a+b*x+c*x*x))-(b/(2.D0*a))* &
      stm31(x,a,b,c)+stm21(x,a,b,c)/a
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm51(x,a,b,c)

      double precision :: x,a,b,c 
  
      stm51=-1.D0/(a*x*(a+b*x+c*x*x))-(2.D0*b/a)*  &
      stm41(x,a,b,c)-(3.D0*c/a)*stm31(x,a,b,c)
