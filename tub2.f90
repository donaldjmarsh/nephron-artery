...
!        local scalars
         ...
         real*8, private, parameter :: bmm2=-0.1631D0
         real*8, private, parameter :: cmm2=-.00294D0
         ...
         integer, parameter, private :: nefid = 2
...
!         private subroutines and functions
          ...
          bitri2    ,BOUND2    ,dfdpg2    ,feedb2   ,     &
          glom2     ,LUBKSB2   ,ludcmp2   ,f2       ,     &
          MNEWT2    ,MMNEWT2   ,nephron2  ,prflcl2  ,     &
          RADIUS2   ,REABGRAD2 ,tubgeom2  ,PDE2     ,     &
          chlorid2  ,stm12     ,stm22     ,stm32    ,     &
          stm42     ,stm52

!! Beginning of tub2 interface.                                                                 
!! ..                                                                                                
!! .. Generic Interface Blocks ..                                                                    
      INTERFACE tub2_F90

!        initall2 downloads all tubular parameters and initial conditions

         MODULE PROCEDURE initall2 

!        calc2 runs the calculations for the tubule

         MODULE PROCEDURE calc2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	xtub2(1)=35.1D0;xtub2(2)=106.9D0;xtub2(3)=55.D0
	xtub2(4)=0.209D0;xtub2(5)=.59D0;xtub2(6)=81.5D0
	xtub2(7:15)=0.0D0
	...
	return
	end subroutine initall2
!!!!!!!!!!!!!!!!!!!!!!!!!
        subroutine calc2(tubTime,jcount)
...
!        ireq(X) X= 2*2
      ...
      itstep=0;euknorm2=1.D0;ras=1.0
      ...
      xt2=feedb2(nacl(imd,1),nacl(imd,2))
	...
	sbf=xtub2(2);signal=xt2

	bufSend2(1)=sbf;bufSend2(2)=signal;bufSend2(3)=gpress


!  ireq(X) X related to send sequence

        call MPI_ISEND(bufSend2,3,MPI_DOUBLE_PRECISION, &
          0,2,MPI_COMM_WORLD,ireq(3),ierr)
	 ...
	 if (jcount.EQ. iprint) then
	     ...
	     ytime=tubTime-startprt
            gfr=xtub2(1);spf=xtub2(2)
            tpress=p(ilen(1),1)*conv1;xmacd=nacl(imd,1)*1.0d03

            write(lout(nefid),'(f8.3,5(2x,g12.4))') ytime ,tpress,xmacd, 2.0D0*spf, &
                 pressNod(nodeNumber(3))/dyn, (nacl(imd,1)+nacl(imd,2))/2.0D0
            endif
          endif 
	endif
!  ireq(X)  related to send sequence

	call MPI_WAIT(ireq(3),status,ierr)
...
! Start iteration loop. 
      ...
      itstep=0;euknorm2=1.D0;sum(3)=1.D0

      do while (((euknorm2.GT.1D-3).OR.(sum(3).GT.1.D-5)) &
...
!
      ...
      call MMNEWT2(ntrial,xtub2,n,tolx,tolf)
...
!  Send message and stop if iteration counter exceeds maxstep.
	...
	enddo
         stop 'itstep exceeds maxstep in nephron2'
      endif
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  newt using the Newton-Raphson method. 
          ...
          call mnewt2(ntrial,x,n,tolx,tolf)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine BOUND2(x,alpha,beta)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MNEWT2(ntrial,x,n,tolx,tolf)
...
!
      ...
      first: do k=1,ntrial
	   ...
	   enddo second
          ...
          call ludcmp2(alpha,n,np,indx) 
          call lubksb2(alpha,n,np,indx,beta)
          errx=0.D0
          third: do i=1,n
	...
	enddo first
        stop 'MNEWT2 exceeding max number of iterations'
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MMNEWT2(ntrial,x,n,tolx,tolf)
      ...
      write (6,*) xp
      stop 'MMNEWT2 exceeding max number of iterations'

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine ludcmp2(a,n,np,indx) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION feedb2(nacl1,nacl2)
...
!     TGF response of macula densa and afferent arteriole
      ...
      feedb2=tgfmax-dtgf/(1.D0+dexp(xgain*(nacl-tpnacl)))

      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION f2(ce,gpf)
...
!      cf=-0.00294D0
      ...
      1.D0/(af*ce)+(bf*bf/(2.D0*af*af)-cf/af)*stm12(ce,af,bf,cf)

      integ2=(bf/(2.D0*af*af))*log((af+bf*ca+cf*ca*ca)/(ca*ca))- &
      1.D0/(af*ca)+(bf*bf/(2.D0*af*af)-cf/af)*stm12(ca,af,bf,cf)

      f2=integ1-integ2-kf/(gpf*ca)


      return
      end 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION dfdpg2(CA,CE)  
...
!  Calculation of integral of 1/C^2*(PG-PT-PI(C))^2 in [CA,CE]
      ...
      a=gpress-pt2
      integ1=stm52(ce,a,bf,cf)
      integ2=stm52(ca,a,bf,cf)
      integ=integ1-integ2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     subroutine glom2(xtub,alfa,beta)
      use affall
...
!    to 0 in this version).
      ...
      beta(2)=gpf-(1.-ha)*(pressNod(nodeNumber(2))/dyn-gpress)/(ras*exp(ks*ha))
      ...
      alfa(6,3)=-dfdpg2(ca,ce)
      alfa(6,6)=-1.D0/(ce*ce*(gpress-pt2-aa*ce-bb*ce*ce))

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      Real*8 FUNCTION stm12(x,a,b,c)
...
!   hvor X = ( a + bx + cx^2 ). q = 4ac-b^2. 
      ...
      stm12=(-1.D0/sqrt(-q))*log((2.D0*c*x+b+sqrt(-q))/ &
       (sqrt(-q)-2.D0*c*x-b))
 
      end
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm22(x,a,b,c)

      real*8 x,a,b,c 

      stm22=(1.D0/(2.D0*a))*log(x*x/(a+b*x+c*x*x))- &
      (b/(2.D0*a))*stm12(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      real*8 FUNCTION stm32(x,a,b,c)
      real*8 x,a,b,c,q
  
      q=4.D0*a*c-b*b
      stm32=(2.D0*c*x+b)/(q*(a+b*x+c*x*x)) + &
      (2.D0*c/q)*stm12(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm42(x,a,b,c)

      real*8 x,a,b,c
  
      stm42=1.D0/(2.D0*a*(a+b*x+c*x*x))-(b/(2.D0*a))* &
      stm32(x,a,b,c)+stm22(x,a,b,c)/a
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm52(x,a,b,c)

      double precision :: x,a,b,c 
  
      stm52=-1.D0/(a*x*(a+b*x+c*x*x))-(2.D0*b/a)*  &
      stm42(x,a,b,c)-(3.D0*c/a)*stm32(x,a,b,c)
