...
!        local scalars
         ...
         real*8, private, parameter :: bmm3=-0.1631D0
         real*8, private, parameter :: cmm3=-.00294D0
         ...
         integer, parameter, private :: nefid = 3
...
!         private subroutines and functions
          ...
          bitri3    ,BOUND3    ,dfdpg3    ,feedb3   ,     &
          glom3     ,LUBKSB3   ,ludcmp3   ,f3       ,     &
          MNEWT3    ,MMNEWT3   ,nephron3  ,prflcl3  ,     &
          RADIUS3   ,REABGRAD3 ,tubgeom3  ,PDE3     ,     &
          chlorid3  ,stm13     ,stm23     ,stm33    ,     &
          stm43     ,stm53

!! Beginning of tub3 interface.                                                                 
!! ..                                                                                                
!! .. Generic Interface Blocks ..                                                                    
      INTERFACE tub3_F90

!        initall3 downloads all tubular parameters and initial conditions

         MODULE PROCEDURE initall3 

!        calc3 runs the calculations for the tubule

         MODULE PROCEDURE calc3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	xtub3(1)=35.1D0;xtub3(2)=106.9D0;xtub3(3)=55.D0
	xtub3(4)=0.209D0;xtub3(5)=.59D0;xtub3(6)=81.5D0
	xtub3(7:15)=0.0D0
	...
	return
	end subroutine initall3
!!!!!!!!!!!!!!!!!!!!!!!!!
        subroutine calc3(tubTime,jcount)
...
!        ireq(X) X= 2*3
      ...
      itstep=0;euknorm3=1.D0;ras=1.0
      ...
      xt2=feedb3(nacl(imd,1),nacl(imd,2))
	...
	sbf=xtub3(2);signal=xt2

	bufSend3(1)=sbf;bufSend3(2)=signal;bufSend3(3)=gpress


!  ireq(X) X related to send sequence

        call MPI_ISEND(bufSend3,3,MPI_DOUBLE_PRECISION, &
          0,3,MPI_COMM_WORLD,ireq(5),ierr)
...
!  ireq(X) X related to send sequence
	...
	call MPI_WAIT(ireq(5),status,ierr)
...
! Start iteration loop. 
      ...
      itstep=0;euknorm3=1.D0;sum(3)=1.D0

      do while (((euknorm3.GT.1D-3).OR.(sum(3).GT.1.D-5)) &
...
!
      ...
      call MMNEWT3(ntrial,xtub3,n,tolx,tolf)
...
!  Send message and stop if iteration counter exceeds maxstep.
	...
	enddo
         stop 'itstep exceeds maxstep in nephron3'
      endif
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  newt using the Newton-Raphson method. 
          ...
          call mnewt3(ntrial,x,n,tolx,tolf)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine BOUND3(x,alpha,beta)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MNEWT3(ntrial,x,n,tolx,tolf)
...
!
      ...
      first: do k=1,ntrial
	   ...
	   enddo second
          ...
          call ludcmp3(alpha,n,np,indx) 
          call lubksb3(alpha,n,np,indx,beta)
          errx=0.D0
          third: do i=1,n
	...
	enddo first
        stop 'MNEWT3 exceeding max number of iterations'
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MMNEWT3(ntrial,x,n,tolx,tolf)
      ...
      write (6,*) xp
      stop 'MMNEWT3 exceeding max number of iterations'

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine ludcmp3(a,n,np,indx) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION feedb3(nacl1,nacl2)
...
!     TGF response of macula densa and afferent arteriole
      ...
      feedb3=tgfmax-dtgf/(1.D0+dexp(xgain*(nacl-tpnacl)))

      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION f3(ce,gpf)
...
!      cf=-0.00294D0
      ...
      1.D0/(af*ce)+(bf*bf/(2.D0*af*af)-cf/af)*stm13(ce,af,bf,cf)

      integ2=(bf/(2.D0*af*af))*log((af+bf*ca+cf*ca*ca)/(ca*ca))- &
      1.D0/(af*ca)+(bf*bf/(2.D0*af*af)-cf/af)*stm13(ca,af,bf,cf)

      f3=integ1-integ2-kf/(gpf*ca)


      return
      end 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION dfdpg3(CA,CE)  
...
!  Calculation of integral of 1/C^2*(PG-PT-PI(C))^2 in [CA,CE]
      ...
      a=gpress-pt3
      integ1=stm53(ce,a,bf,cf)
      integ2=stm53(ca,a,bf,cf)
      integ=integ1-integ2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     subroutine glom3(xtub,alfa,beta)
      use affall
...
!    to 0 in this version).
      ...
      beta(2)=gpf-(1.-ha)*(pressNod(nodeNumber(3))/dyn-gpress)/(ras*exp(ks*ha))
      ...
      alfa(6,3)=-dfdpg3(ca,ce)
      alfa(6,6)=-1.D0/(ce*ce*(gpress-pt3-aa*ce-bb*ce*ce))

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      Real*8 FUNCTION stm13(x,a,b,c)
...
!   hvor X = ( a + bx + cx^2 ). q = 4ac-b^2. 
      ...
      stm13=(-1.D0/sqrt(-q))*log((2.D0*c*x+b+sqrt(-q))/ &
       (sqrt(-q)-2.D0*c*x-b))
 
      end
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm23(x,a,b,c)

      real*8 x,a,b,c 

      stm23=(1.D0/(2.D0*a))*log(x*x/(a+b*x+c*x*x))- &
      (b/(2.D0*a))*stm13(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      real*8 FUNCTION stm33(x,a,b,c)
      real*8 x,a,b,c,q
  
      q=4.D0*a*c-b*b
      stm33=(2.D0*c*x+b)/(q*(a+b*x+c*x*x)) + &
      (2.D0*c/q)*stm13(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm43(x,a,b,c)

      real*8 x,a,b,c
  
      stm43=1.D0/(2.D0*a*(a+b*x+c*x*x))-(b/(2.D0*a))* &
      stm33(x,a,b,c)+stm23(x,a,b,c)/a
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm53(x,a,b,c)

      double precision :: x,a,b,c 
  
      stm53=-1.D0/(a*x*(a+b*x+c*x*x))-(2.D0*b/a)*  &
      stm43(x,a,b,c)-(3.D0*c/a)*stm33(x,a,b,c)
