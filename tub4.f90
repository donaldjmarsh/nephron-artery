...
!        local scalars
         ...
         real*8, private, parameter :: bmm4=-0.1631D0
         real*8, private, parameter :: cmm4=-.00294D0
         ...
         integer, parameter, private :: nefid = 4
...
!         private subroutines and functions
          ...
          bitri4    ,BOUND4    ,dfdpg4    ,feedb4   ,     &
          glom4     ,LUBKSB4   ,ludcmp4   ,f4       ,     &
          MNEWT4    ,MMNEWT4   ,nephron4  ,prflcl4  ,     &
          RADIUS4   ,REABGRAD4 ,tubgeom4  ,PDE4     ,     &
          chlorid4  ,stm14     ,stm24     ,stm34    ,     &
          stm44     ,stm54

!! Beginning of tub4 interface.                                                                 
!! ..                                                                                                
!! .. Generic Interface Blocks ..                                                                    
      INTERFACE tub4_F90

!        initall4 downloads all tubular parameters and initial conditions

         MODULE PROCEDURE initall4 

!        calc4 runs the calculations for the tubule
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	xtub4(1)=35.1D0;xtub4(2)=106.9D0;xtub4(3)=55.D0
	xtub4(4)=0.209D0;xtub4(5)=.59D0;xtub4(6)=81.5D0
	xtub4(7:15)=0.0D0
	...
	return
	end subroutine initall4
!!!!!!!!!!!!!!!!!!!!!!!!!
        subroutine calc4(tubTime,jcount)
...
!        ireq(X) X= 2*4
      ...
      itstep=0;euknorm4=1.D0;ras=1.0
      ...
      xt2=feedb4(nacl(imd,1),nacl(imd,2))
	...
	sbf=xtub4(2);signal=xt2

	bufSend4(1)=sbf;bufSend4(2)=signal;bufSend4(3)=gpress


!  ireq(X) X related to send sequence

        call MPI_ISEND(bufSend4,3,MPI_DOUBLE_PRECISION, &
          0,4,MPI_COMM_WORLD,ireq(7),ierr)
	 ...
	 if (jcount.EQ. iprint) then
	     ...
	     ytime=tubTime-startprt
            gfr=xtub4(1);spf=xtub4(2)
            tpress=p(ilen(1),1)*conv1;xmacd=nacl(imd,1)*1.0d03

            write(lout(nefid),'(f8.3,5(2x,g12.4))') ytime ,tpress,xmacd,2.0D0*spf,  &
                 pressNod(nodeNumber(4))/dyn,(nacl(imd,1)+nacl(imd,2))/2.0D0
          endif 
         endif       
	endif
!  ireq(X) X related to send sequence

	call MPI_WAIT(ireq(7),status,ierr)
...
! Start iteration loop. 
      ...
      itstep=0;euknorm4=1.D0;sum(3)=1.D0

      do while (((euknorm4.GT.1D-3).OR.(sum(3).GT.1.D-5)) &
...
!
      ...
      call MMNEWT4(ntrial,xtub4,n,tolx,tolf)
...
!  Send message and stop if iteration counter exceeds maxstep.
	...
	enddo
         stop 'itstep exceeds maxstep in nephron4'
      endif
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  newt using the Newton-Raphson method. 
          ...
          call mnewt4(ntrial,x,n,tolx,tolf)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine BOUND4(x,alpha,beta)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MNEWT4(ntrial,x,n,tolx,tolf)
...
!
      ...
      first: do k=1,ntrial
	   ...
	   enddo second
          ...
          call ludcmp4(alpha,n,np,indx) 
          call lubksb4(alpha,n,np,indx,beta)
          errx=0.D0
          third: do i=1,n
	...
	enddo first
        stop 'MNEWT4 exceeding max number of iterations'
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MMNEWT4(ntrial,x,n,tolx,tolf)
      ...
      write (6,*) xp
      stop 'MMNEWT4 exceeding max number of iterations'

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine ludcmp4(a,n,np,indx) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine LUBKSB4(a,n,np,indx,b)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION feedb4(nacl1,nacl2)
...
!     TGF response of macula densa and afferent arteriole
      ...
      feedb4=tgfmax-dtgf/(1.D0+dexp(xgain*(nacl-tpnacl)))

      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION f4(ce,gpf)
...
!      cf=-0.00294D0
      ...
      1.D0/(af*ce)+(bf*bf/(2.D0*af*af)-cf/af)*stm14(ce,af,bf,cf)

      integ2=(bf/(2.D0*af*af))*log((af+bf*ca+cf*ca*ca)/(ca*ca))- &
      1.D0/(af*ca)+(bf*bf/(2.D0*af*af)-cf/af)*stm14(ca,af,bf,cf)

      f4=integ1-integ2-kf/(gpf*ca)


      return
      end 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION dfdpg4(CA,CE)  
...
!  Calculation of integral of 1/C^2*(PG-PT-PI(C))^2 in [CA,CE]
      ...
      a=gpress-pt4
      integ1=stm54(ce,a,bf,cf)
      integ2=stm54(ca,a,bf,cf)
      integ=integ1-integ2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     subroutine glom4(xtub,alfa,beta)
      use affall
...
!    to 0 in this version).
      ...
      beta(2)=gpf-(1.-ha)*(pressNod(nodeNumber(4))/dyn-gpress)/(ras*exp(ks*ha))
      ...
      alfa(6,3)=-dfdpg4(ca,ce)
      alfa(6,6)=-1.D0/(ce*ce*(gpress-pt4-aa*ce-bb*ce*ce))

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      Real*8 FUNCTION stm14(x,a,b,c)
...
!   hvor X = ( a + bx + cx^2 ). q = 4ac-b^2. 
      ...
      stm14=(-1.D0/sqrt(-q))*log((2.D0*c*x+b+sqrt(-q))/ &
       (sqrt(-q)-2.D0*c*x-b))
 
      end
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm24(x,a,b,c)

      real*8 x,a,b,c 

      stm24=(1.D0/(2.D0*a))*log(x*x/(a+b*x+c*x*x))- &
      (b/(2.D0*a))*stm14(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      real*8 FUNCTION stm34(x,a,b,c)
      real*8 x,a,b,c,q
  
      q=4.D0*a*c-b*b
      stm34=(2.D0*c*x+b)/(q*(a+b*x+c*x*x)) + &
      (2.D0*c/q)*stm14(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm44(x,a,b,c)

      real*8 x,a,b,c
  
      stm44=1.D0/(2.D0*a*(a+b*x+c*x*x))-(b/(2.D0*a))* &
      stm34(x,a,b,c)+stm24(x,a,b,c)/a
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm54(x,a,b,c)

      double precision :: x,a,b,c 
  
      stm54=-1.D0/(a*x*(a+b*x+c*x*x))-(2.D0*b/a)*  &
      stm44(x,a,b,c)-(3.D0*c/a)*stm34(x,a,b,c)
