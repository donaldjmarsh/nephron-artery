...
!        local scalars
         ...
         real*8, private, parameter :: bmm5=-0.1631D0
         real*8, private, parameter :: cmm5=-.00294D0
         ...
         integer, parameter, private :: nefid = 5
...
!         private subroutines and functions
          ...
          bitri5    ,BOUND5    ,dfdpg5    ,feedb5   ,     &
          glom5     ,LUBKSB5   ,ludcmp5   ,f5       ,     &
          MNEWT5    ,MMNEWT5   ,nephron5  ,prflcl5  ,     &
          RADIUS5   ,REABGRAD5 ,tubgeom5  ,PDE5     ,     &
          chlorid5  ,stm15     ,stm25     ,stm35    ,     &
          stm45     ,stm55

!! Beginning of tub5 interface.                                                                 
!! ..                                                                                                
!! .. Generic Interface Blocks ..                                                                    
      INTERFACE tub5_F90

!        initall5 downloads all tubular parameters and initial conditions

         MODULE PROCEDURE initall5 

!        calc5 runs the calculations for the tubule

         MODULE PROCEDURE calc5
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	xtub5(1)=35.1D0;xtub5(2)=106.9D0;xtub5(3)=55.D0
	xtub5(4)=0.209D0;xtub5(5)=.59D0;xtub5(6)=81.5D0
	xtub5(7:15)=0.0D0
	...
	return
	end subroutine initall5
!!!!!!!!!!!!!!!!!!!!!!!!!
        subroutine calc5(tubTime,jcount)
...
!        ireq(X) X= 2*5
      ...
      itstep=0;euknorm5=1.D0;ras=1.0
      ...
      xt2=feedb5(nacl(imd,1),nacl(imd,2))
	...
	sbf=xtub5(2);signal=xt2

	bufSend5(1)=sbf;bufSend5(2)=signal;bufSend5(3)=gpress


!  ireq(X) X related to send sequence

        call MPI_ISEND(bufSend5,3,MPI_DOUBLE_PRECISION, &
          0,5,MPI_COMM_WORLD,ireq(9),ierr)
...
!  ireq(X) X related to send sequence
	...
	call MPI_WAIT(ireq(9),status,ierr)
...
! Start iteration loop. 
      ...
      itstep=0;euknorm5=1.D0;sum(3)=1.D0

      do while (((euknorm5.GT.1D-3).OR.(sum(3).GT.1.D-5)) &
...
!
      ...
      call MMNEWT5(ntrial,xtub5,n,tolx,tolf)
...
!  Send message and stop if iteration counter exceeds maxstep.
	...
	enddo
         stop 'itstep exceeds maxstep in nephron5'
      endif
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  newt using the Newton-Raphson method. 
          ...
          call mnewt5(ntrial,x,n,tolx,tolf)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine BOUND5(x,alpha,beta)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MNEWT5(ntrial,x,n,tolx,tolf)
...
!
      ...
      first: do k=1,ntrial
	   ...
	   enddo second
          ...
          call ludcmp5(alpha,n,np,indx) 
          call lubksb5(alpha,n,np,indx,beta)
          errx=0.D0
          third: do i=1,n
	...
	enddo first
        stop 'MNEWT5 exceeding max number of iterations'
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MMNEWT5(ntrial,x,n,tolx,tolf)
      ...
      write (6,*) xp
      stop 'MMNEWT5 exceeding max number of iterations'

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine ludcmp5(a,n,np,indx) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine LUBKSB5(a,n,np,indx,b)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION feedb5(nacl1,nacl2)
...
!     TGF response of macula densa and afferent arteriole
      ...
      feedb5=tgfmax-dtgf/(1.D0+dexp(xgain*(nacl-tpnacl)))

      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION f5(ce,gpf)
...
!      cf=-0.00294D0
      ...
      1.D0/(af*ce)+(bf*bf/(2.D0*af*af)-cf/af)*stm15(ce,af,bf,cf)

      integ2=(bf/(2.D0*af*af))*log((af+bf*ca+cf*ca*ca)/(ca*ca))- &
      1.D0/(af*ca)+(bf*bf/(2.D0*af*af)-cf/af)*stm15(ca,af,bf,cf)

      f5=integ1-integ2-kf/(gpf*ca)


      return
      end 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION dfdpg5(CA,CE)  
...
!  Calculation of integral of 1/C^2*(PG-PT-PI(C))^2 in [CA,CE]
      ...
      a=gpress-pt5
      integ1=stm55(ce,a,bf,cf)
      integ2=stm55(ca,a,bf,cf)
      integ=integ1-integ2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     subroutine glom5(xtub,alfa,beta)
      use affall
...
!    to 0 in this version).
      ...
      beta(2)=gpf-(1.-ha)*(pressNod(nodeNumber(5))/dyn-gpress)/(ras*exp(ks*ha))
      ...
      alfa(6,3)=-dfdpg5(ca,ce)
      alfa(6,6)=-1.D0/(ce*ce*(gpress-pt5-aa*ce-bb*ce*ce))

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      Real*8 FUNCTION stm15(x,a,b,c)
...
!   hvor X = ( a + bx + cx^2 ). q = 4ac-b^2. 
      ...
      stm15=(-1.D0/sqrt(-q))*log((2.D0*c*x+b+sqrt(-q))/ &
       (sqrt(-q)-2.D0*c*x-b))
 
      end
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm25(x,a,b,c)

      real*8 x,a,b,c 

      stm25=(1.D0/(2.D0*a))*log(x*x/(a+b*x+c*x*x))- &
      (b/(2.D0*a))*stm15(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      real*8 FUNCTION stm35(x,a,b,c)
      real*8 x,a,b,c,q
  
      q=4.D0*a*c-b*b
      stm35=(2.D0*c*x+b)/(q*(a+b*x+c*x*x)) + &
      (2.D0*c/q)*stm15(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm45(x,a,b,c)

      real*8 x,a,b,c
  
      stm45=1.D0/(2.D0*a*(a+b*x+c*x*x))-(b/(2.D0*a))* &
      stm35(x,a,b,c)+stm25(x,a,b,c)/a
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm55(x,a,b,c)

      double precision :: x,a,b,c 
  
      stm55=-1.D0/(a*x*(a+b*x+c*x*x))-(2.D0*b/a)*  &
      stm45(x,a,b,c)-(3.D0*c/a)*stm35(x,a,b,c)
