...
!        local scalars
         ...
         real*8, private, parameter :: bmm7=-0.1631D0
         real*8, private, parameter :: cmm7=-.00294D0
         ...
         integer, parameter, private :: nefid = 7
...
!         private subroutines and functions
          ...
          bitri7    ,BOUND7    ,dfdpg7    ,feedb7   ,     &
          glom7     ,LUBKSB7   ,ludcmp7   ,f7       ,     &
          MNEWT7    ,MMNEWT7   ,nephron7  ,prflcl7  ,     &
          RADIUS7   ,REABGRAD7 ,tubgeom7  ,PDE7     ,     &
          chlorid7  ,stm17     ,stm27     ,stm37    ,     &
          stm47     ,stm57

!! Beginning of tub7 interface.                                                                 
!! ..                                                                                                
!! .. Generic Interface Blocks ..                                                                    
      INTERFACE tub7_F90

!        initall7 downloads all tubular parameters and initial conditions

         MODULE PROCEDURE initall7 

!        calc7 runs the calculations for the tubule

         MODULE PROCEDURE calc7
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	xtub7(1)=35.1D0;xtub7(2)=106.9D0;xtub7(3)=55.D0
	xtub7(4)=0.209D0;xtub7(5)=.59D0;xtub7(6)=81.5D0
	xtub7(7:15)=0.0D0
	...
	return
	end subroutine initall7
!!!!!!!!!!!!!!!!!!!!!!!!!
        subroutine calc7(tubTime,jcount)
...
!        ireq(X) X= 2*7
      ...
      itstep=0;euknorm7=1.D0;ras=1.0
      ...
      xt2=feedb7(nacl(imd,1),nacl(imd,2))
	...
	sbf=xtub7(2);signal=xt2

	bufSend7(1)=sbf;bufSend7(2)=signal;bufSend7(3)=gpress


!  ireq(X) X related to send sequence

        call MPI_ISEND(bufSend7,3,MPI_DOUBLE_PRECISION, &
          0,7,MPI_COMM_WORLD,ireq(13),ierr)
...
!  ireq(X) X related to send sequence
	...
	call MPI_WAIT(ireq(13),status,ierr)
...
! Start iteration loop. 
      ...
      itstep=0;euknorm7=1.D0;sum(3)=1.D0

      do while (((euknorm7.GT.1D-3).OR.(sum(3).GT.1.D-5)) &
...
!
      ...
      call MMNEWT7(ntrial,xtub7,n,tolx,tolf)
...
!  Send message and stop if iteration counter exceeds maxstep.
	...
	enddo
         stop 'itstep exceeds maxstep in nephron7'
      endif
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  newt using the Newton-Raphson method. 
          ...
          call mnewt7(ntrial,x,n,tolx,tolf)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine BOUND7(x,alpha,beta)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MNEWT7(ntrial,x,n,tolx,tolf)
...
!
      ...
      first: do k=1,ntrial
	   ...
	   enddo second
          ...
          call ludcmp7(alpha,n,np,indx) 
          call lubksb7(alpha,n,np,indx,beta)
          errx=0.D0
          third: do i=1,n
	...
	enddo first
        stop 'MNEWT7 exceeding max number of iterations'
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MMNEWT7(ntrial,x,n,tolx,tolf)
      ...
      write (6,*) xp
      stop 'MMNEWT7 exceeding max number of iterations'

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine ludcmp7(a,n,np,indx) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine LUBKSB7(a,n,np,indx,b)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION feedb7(nacl1,nacl2)
...
!     TGF response of macula densa and afferent arteriole
      ...
      feedb7=tgfmax-dtgf/(1.D0+dexp(xgain*(nacl-tpnacl)))

      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION f7(ce,gpf)
...
!      cf=-0.00294D0
      ...
      1.D0/(af*ce)+(bf*bf/(2.D0*af*af)-cf/af)*stm17(ce,af,bf,cf)

      integ2=(bf/(2.D0*af*af))*log((af+bf*ca+cf*ca*ca)/(ca*ca))- &
      1.D0/(af*ca)+(bf*bf/(2.D0*af*af)-cf/af)*stm17(ca,af,bf,cf)

      f7=integ1-integ2-kf/(gpf*ca)


      return
      end 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION dfdpg7(CA,CE)  
...
!  Calculation of integral of 1/C^2*(PG-PT-PI(C))^2 in [CA,CE]
      ...
      a=gpress-pt7
      integ1=stm57(ce,a,bf,cf)
      integ2=stm57(ca,a,bf,cf)
      integ=integ1-integ2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     subroutine glom7(xtub,alfa,beta)
      use affall
...
!    to 0 in this version).
      ...
      beta(2)=gpf-(1.-ha)*(pressNod(nodeNumber(7))/dyn-gpress)/(ras*exp(ks*ha))
      ...
      alfa(6,3)=-dfdpg7(ca,ce)
      alfa(6,6)=-1.D0/(ce*ce*(gpress-pt7-aa*ce-bb*ce*ce))

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      Real*8 FUNCTION stm17(x,a,b,c)
...
!   hvor X = ( a + bx + cx^2 ). q = 4ac-b^2. 
      ...
      stm17=(-1.D0/sqrt(-q))*log((2.D0*c*x+b+sqrt(-q))/ &
       (sqrt(-q)-2.D0*c*x-b))
 
      end
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm27(x,a,b,c)

      real*8 x,a,b,c 

      stm27=(1.D0/(2.D0*a))*log(x*x/(a+b*x+c*x*x))- &
      (b/(2.D0*a))*stm17(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      real*8 FUNCTION stm37(x,a,b,c)
      real*8 x,a,b,c,q
  
      q=4.D0*a*c-b*b
      stm37=(2.D0*c*x+b)/(q*(a+b*x+c*x*x)) + &
      (2.D0*c/q)*stm17(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm47(x,a,b,c)

      real*8 x,a,b,c
  
      stm47=1.D0/(2.D0*a*(a+b*x+c*x*x))-(b/(2.D0*a))* &
      stm37(x,a,b,c)+stm27(x,a,b,c)/a
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm57(x,a,b,c)

      double precision :: x,a,b,c 
  
      stm57=-1.D0/(a*x*(a+b*x+c*x*x))-(2.D0*b/a)*  &
      stm47(x,a,b,c)-(3.D0*c/a)*stm37(x,a,b,c)
