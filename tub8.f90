...
!        local scalars
         ...
         real*8, private, parameter :: bmm8=-0.1631D0
         real*8, private, parameter :: cmm8=-.00294D0
         ...
         integer, parameter, private :: nefid = 8
...
!         private subroutines and functions
          ...
          bitri8    ,BOUND8    ,dfdpg8    ,feedb8   ,     &
          glom8     ,LUBKSB8   ,ludcmp8   ,f8       ,     &
          MNEWT8    ,MMNEWT8   ,nephron8  ,prflcl8  ,     &
          RADIUS8   ,REABGRAD8 ,tubgeom8  ,PDE8     ,     &
          chlorid8  ,stm18     ,stm28     ,stm38    ,     &
          stm48     ,stm58

!! Beginning of tub8 interface.                                                                 
!! ..                                                                                                
!! .. Generic Interface Blocks ..                                                                    
      INTERFACE tub8_F90

!        initall8 downloads all tubular parameters and initial conditions

         MODULE PROCEDURE initall8 

!        calc8 runs the calculations for the tubule
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	xtub8(1)=35.1D0;xtub8(2)=106.9D0;xtub8(3)=55.D0
	xtub8(4)=0.209D0;xtub8(5)=.59D0;xtub8(6)=81.5D0
	xtub8(7:15)=0.0D0
	...
	return
	end subroutine initall8
!!!!!!!!!!!!!!!!!!!!!!!!!
        subroutine calc8(tubTime,jcount)
...
!        ireq(X) X= 2*8
      ...
      itstep=0;euknorm8=1.D0;ras=1.0
      ...
      xt2=feedb8(nacl(imd,1),nacl(imd,2))
	...
	sbf=xtub8(2);signal=xt2

	bufSend8(1)=sbf;bufSend8(2)=signal;bufSend8(3)=gpress


!  ireq(X) X related to send sequence

        call MPI_ISEND(bufSend8,3,MPI_DOUBLE_PRECISION, &
          0,8,MPI_COMM_WORLD,ireq(15),ierr)
...
!  ireq(X) X related to send sequence
	...
	call MPI_WAIT(ireq(15),status,ierr)
...
! Start iteration loop. 
      ...
      itstep=0;euknorm8=1.D0;sum(3)=1.D0

      do while (((euknorm8.GT.1D-3).OR.(sum(3).GT.1.D-5)) &
...
!
      ...
      call MMNEWT8(ntrial,xtub8,n,tolx,tolf)
...
!  Send message and stop if iteration counter exceeds maxstep.
	...
	enddo
         stop 'itstep exceeds maxstep in nephron8'
      endif
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  newt using the Newton-Raphson method. 
          ...
          call mnewt8(ntrial,x,n,tolx,tolf)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine BOUND8(x,alpha,beta)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MNEWT8(ntrial,x,n,tolx,tolf)
...
!
      ...
      first: do k=1,ntrial
	   ...
	   enddo second
          ...
          call ludcmp8(alpha,n,np,indx) 
          call lubksb8(alpha,n,np,indx,beta)
          errx=0.D0
          third: do i=1,n
	...
	enddo first
        stop 'MNEWT8 exceeding max number of iterations'
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MMNEWT8(ntrial,x,n,tolx,tolf)
      ...
      write (6,*) xp
      stop 'MMNEWT8 exceeding max number of iterations'

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine ludcmp8(a,n,np,indx) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION feedb8(nacl1,nacl2)
...
!     TGF response of macula densa and afferent arteriole
      ...
      feedb8=tgfmax-dtgf/(1.D0+dexp(xgain*(nacl-tpnacl)))

      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION f8(ce,gpf)
...
!      cf=-0.00294D0
      ...
      1.D0/(af*ce)+(bf*bf/(2.D0*af*af)-cf/af)*stm18(ce,af,bf,cf)

      integ2=(bf/(2.D0*af*af))*log((af+bf*ca+cf*ca*ca)/(ca*ca))- &
      1.D0/(af*ca)+(bf*bf/(2.D0*af*af)-cf/af)*stm18(ca,af,bf,cf)

      f8=integ1-integ2-kf/(gpf*ca)

      end 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION dfdpg8(CA,CE)  
...
!  Calculation of integral of 1/C^2*(PG-PT-PI(C))^2 in [CA,CE]
      ...
      a=gpress-pt8
      integ1=stm58(ce,a,bf,cf)
      integ2=stm58(ca,a,bf,cf)
      integ=integ1-integ2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     subroutine glom8(xtub,alfa,beta)
      use affall
...
!    to 0 in this version).
      ...
      beta(2)=gpf-(1.-ha)*(pressNod(nodeNumber(8))/dyn-gpress)/(ras*exp(ks*ha))
      ...
      alfa(6,3)=-dfdpg8(ca,ce)
      alfa(6,6)=-1.D0/(ce*ce*(gpress-pt8-aa*ce-bb*ce*ce))

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      Real*8 FUNCTION stm18(x,a,b,c)
...
!   hvor X = ( a + bx + cx^2 ). q = 4ac-b^2. 
      ...
      stm18=(-1.D0/sqrt(-q))*log((2.D0*c*x+b+sqrt(-q))/ &
       (sqrt(-q)-2.D0*c*x-b))
 
      end
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm28(x,a,b,c)

      real*8 x,a,b,c 

      stm28=(1.D0/(2.D0*a))*log(x*x/(a+b*x+c*x*x))- &
      (b/(2.D0*a))*stm18(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      real*8 FUNCTION stm38(x,a,b,c)
      real*8 x,a,b,c,q
  
      q=4.D0*a*c-b*b
      stm38=(2.D0*c*x+b)/(q*(a+b*x+c*x*x)) + &
      (2.D0*c/q)*stm18(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm48(x,a,b,c)

      real*8 x,a,b,c
  
      stm48=1.D0/(2.D0*a*(a+b*x+c*x*x))-(b/(2.D0*a))* &
      stm38(x,a,b,c)+stm28(x,a,b,c)/a
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm58(x,a,b,c)

      double precision :: x,a,b,c 
  
      stm58=-1.D0/(a*x*(a+b*x+c*x*x))-(2.D0*b/a)*  &
      stm48(x,a,b,c)-(3.D0*c/a)*stm38(x,a,b,c)
