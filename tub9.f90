...
!        local scalars
         ...
         real*8, private, parameter :: bmm9=-0.1631D0
         real*8, private, parameter :: cmm9=-.00294D0
         ...
         integer, parameter, private :: nefid = 9
...
!         private subroutines and functions
          ...
          bitri9    ,BOUND9    ,dfdpg9    ,feedb9   ,     &
          glom9     ,LUBKSB9   ,ludcmp9   ,f9       ,     &
          MNEWT9    ,MMNEWT9   ,nephron9  ,prflcl9  ,     &
          RADIUS9   ,REABGRAD9 ,tubgeom9  ,PDE9     ,     &
          chlorid9  ,stm19     ,stm29     ,stm39    ,     &
          stm49     ,stm59

!! Beginning of tub9 interface.                                                                 
!! ..                                                                                                
!! .. Generic Interface Blocks ..                                                                    
      INTERFACE tub9_F90

!        initall9 downloads all tubular parameters and initial conditions

         MODULE PROCEDURE initall9 

!        calc9 runs the calculations for the tubule

         MODULE PROCEDURE calc9
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	xtub9(1)=35.1D0;xtub9(2)=106.9D0;xtub9(3)=55.D0
	xtub9(4)=0.209D0;xtub9(5)=.59D0;xtub9(6)=81.5D0
	xtub9(7:15)=0.0D0
	...
	return
	end subroutine initall9
!!!!!!!!!!!!!!!!!!!!!!!!!
        subroutine calc9(tubTime,jcount)
...
!        ireq(X) X= 2*9
      ...
      itstep=0;euknorm9=1.D0;ras=1.0
      ...
      xt2=feedb9(nacl(imd,1),nacl(imd,2))
	...
	sbf=xtub9(2);signal=xt2

	bufSend9(1)=sbf;bufSend9(2)=signal;bufSend9(3)=gpress


!  ireq(X) X related to send sequence

        call MPI_ISEND(bufSend9,3,MPI_DOUBLE_PRECISION, &
          0,9,MPI_COMM_WORLD,ireq(17),ierr)
...
!  ireq(X) X related to send sequence
	...
	call MPI_WAIT(ireq(17),status,ierr)
...
! Start iteration loop. 
      ...
      itstep=0;euknorm9=1.D0;sum(3)=1.D0

      do while (((euknorm9.GT.1D-3).OR.(sum(3).GT.1.D-5)) &
...
!
      ...
      call MMNEWT9(ntrial,xtub9,n,tolx,tolf)
...
!  Send message and stop if iteration counter exceeds maxstep.
	...
	enddo
         stop 'itstep exceeds maxstep in nephron9'
      endif
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  newt using the Newton-Raphson method. 
          ...
          call mnewt9(ntrial,x,n,tolx,tolf)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine BOUND9(x,alpha,beta)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MNEWT9(ntrial,x,n,tolx,tolf)
...
!
      ...
      first: do k=1,ntrial
	   ...
	   enddo second
          ...
          call ludcmp9(alpha,n,np,indx) 
          call lubksb9(alpha,n,np,indx,beta)
          errx=0.D0
          third: do i=1,n
	...
	enddo first
        stop 'MNEWT9 exceeding max number of iterations'
      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine MMNEWT9(ntrial,x,n,tolx,tolf)
      ...
      write (6,*) xp
      stop 'MMNEWT9 exceeding max number of iterations'

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine ludcmp9(a,n,np,indx) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine LUBKSB9(a,n,np,indx,b)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION feedb9(nacl1,nacl2)
...
!     TGF response of macula densa and afferent arteriole
      ...
      feedb9=tgfmax-dtgf/(1.D0+dexp(xgain*(nacl-tpnacl)))

      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION f9(ce,gpf)
...
!      cf=-0.00294D0
      ...
      1.D0/(af*ce)+(bf*bf/(2.D0*af*af)-cf/af)*stm19(ce,af,bf,cf)

      integ2=(bf/(2.D0*af*af))*log((af+bf*ca+cf*ca*ca)/(ca*ca))- &
      1.D0/(af*ca)+(bf*bf/(2.D0*af*af)-cf/af)*stm19(ca,af,bf,cf)

      f9=integ1-integ2-kf/(gpf*ca)


      return
      end 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION dfdpg9(CA,CE)  
...
!  Calculation of integral of 1/C^2*(PG-PT-PI(C))^2 in [CA,CE]
      ...
      a=gpress-pt9
      integ1=stm59(ce,a,bf,cf)
      integ2=stm59(ca,a,bf,cf)
      integ=integ1-integ2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     subroutine glom9(xtub,alfa,beta)
      use affall
...
!    to 0 in this version).
      ...
      beta(2)=gpf-(1.-ha)*(pressNod(nodeNumber(9))/dyn-gpress)/(ras*exp(ks*ha))
      ...
      alfa(6,3)=-dfdpg9(ca,ce)
      alfa(6,6)=-1.D0/(ce*ce*(gpress-pt9-aa*ce-bb*ce*ce))

      return
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      Real*8 FUNCTION stm19(x,a,b,c)
...
!   hvor X = ( a + bx + cx^2 ). q = 4ac-b^2. 
      ...
      stm19=(-1.D0/sqrt(-q))*log((2.D0*c*x+b+sqrt(-q))/ &
       (sqrt(-q)-2.D0*c*x-b))
 
      end
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm29(x,a,b,c)

      real*8 x,a,b,c 

      stm29=(1.D0/(2.D0*a))*log(x*x/(a+b*x+c*x*x))- &
      (b/(2.D0*a))*stm19(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      real*8 FUNCTION stm39(x,a,b,c)
      real*8 x,a,b,c,q
  
      q=4.D0*a*c-b*b
      stm39=(2.D0*c*x+b)/(q*(a+b*x+c*x*x)) + &
      (2.D0*c/q)*stm19(x,a,b,c)
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm49(x,a,b,c)

      real*8 x,a,b,c
  
      stm49=1.D0/(2.D0*a*(a+b*x+c*x*x))-(b/(2.D0*a))* &
      stm39(x,a,b,c)+stm29(x,a,b,c)/a
  
      end
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 FUNCTION stm59(x,a,b,c)

      double precision :: x,a,b,c 
  
      stm59=-1.D0/(a*x*(a+b*x+c*x*x))-(2.D0*b/a)*  &
      stm49(x,a,b,c)-(3.D0*c/a)*stm39(x,a,b,c)
